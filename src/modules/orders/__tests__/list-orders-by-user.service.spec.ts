...
import { Paginated } from 'src/@shared/infra/database/repositories/products-repository.interface'; // Assuming Paginated is shared
...
describe('ListOrdersByUserService', () => {
  ...
  beforeEach(async () => {
    ...
    ordersRepository = module.get<InMemoryOrdersRepository>('OrdersRepositoryInterface');
  ...
  it('should return an empty array if no orders exist for the user', async () => {
    const result = await service.execute({ userId: 'user-id-1', page: 1, limit: 10 });
  ...
  it('should return orders for a specific user', async () => {
    ...
    const order1 = Order.create({ userId, total: 10, status: OrderStatus.PENDING, products: [] });
    const order2 = Order.create({ userId, total: 20, status: OrderStatus.SUCCEEDED, products: [] });
    const order3 = Order.create({ userId: 'user-id-2', total: 30, status: OrderStatus.PENDING, products: [] });
    ...
    expect(result.items.map(o => o.id)).toEqual(expect.arrayContaining([order1.id, order2.id]));
  });

  it('should return paginated orders for a specific user', async () => {
    const userId = 'user-id-1';
    for (let i = 1; i <= 5; i++) {
      await ordersRepository.save(Order.create({ userId, total: i * 10, status: OrderStatus.PENDING, products: [] }));
    }
